...
On the primary seed (s0, the archival record; regenerable to the third decimal from the frozen checkpoints):

| rung | $\rho(V^*)$ | $\rho(V^\pi)$ | tracks environment over habit? |
|---|---|---|---|
| $V$ | 0.665 | 0.841 | no |
| $V^{\text{exp}}$ | 0.665 | 0.829 | no |
| $V^\dagger$ | 0.577 | 0.817 | no |
| $V^\diamond$ | **0.698** | 0.679 | **yes** |
...
**Seed robustness (added 2026-08-01, and it bounds the claim).** Re-running the identical pipeline on the three remaining base policies (same 4M-step stage; their transition dumps regenerated from the frozen policies, fit-seed sensitivity separately controlled — s0 spread 0.675–0.708 over three fit seeds, so the differences below are policy-seed effects, not fit noise):

| seed | $V$ | $V^{\text{exp}}$ | $V^\dagger$ | $V^\diamond$ | $V^\diamond$ $\rho(V^\pi)$ |
|---|---|---|---|---|---|
| s0 | 0.665 | 0.665 | 0.577 | **0.698** | 0.679 |
| s1 | 0.609 | 0.648 | **0.684** | 0.418 | 0.089 |
| s2 | 0.601 | 0.677 | 0.563 | 0.408 | 0.042 |
| s3 | 0.485 | 0.511 | 0.604 | 0.541 | 0.138 |

What replicates on all four seeds is the **decorrelation property**: $V^\diamond$'s $\rho(V^\pi)$ sits at 0.04–0.68 against ≥ 0.59 for every record rung, and it beats habit on 4/4 seeds (marginally on one s1 resample; $V^\dagger$ also beats habit on s1, so "the *only* rung" fails there). What does **not** replicate is absolute oracle accuracy: "highest $\rho(V^*)$ of the four" holds on s0 only. The candidate explanation, untested: s0 is much the strongest of the four policies (touch 0.150 vs 0.075/0.099/0.034), so its record gives $\hat r$ and $\Sigma$ the most reward signal to read — detection quality may track record richness, consistent with §11.6. The rung's defensible cross-seed claim is therefore *it reads a different signal than the record rungs*, not *it reads the oracle more accurately* — and the actuation results (§7), which are the behavioral evidence, are measured independently of this table.
...
5. **Detection accuracy is seed-dependent and was validated on young policies.** Across four 4M-step base seeds the beats-habit property replicates but absolute $\rho(V^*)$ ranges 0.41–0.70, apparently tracking record richness (§6.4); and whether the property degrades as remaining headroom becomes fine-grained is untested — the resolution argument (§10) suggests it may.
