...
|---|---|---|---|---|
| [LEAGUE_LORA.md](LEAGUE_LORA.md) | 08-26 | league arm off 7.9-gco @ 442B | LoRA-variant league + discriminator diversity reward. Variants train **strong** (0.42-0.47 vs main) but never **diverse**: kappa .089-.108 across 5 configs (size 8/3, beta 1%-128% of adv scale, 2 descriptors). Beta/size/descriptor/rank all falsified; **symmetry breaking (B=0 at birth) specced + built, UNTESTED** — blocked on an el8 outage. Frozen control (B=0) pins 0.5 with disc AT CHANCE | **RESULT (negative on diversity)** |
