> **Status: HISTORICAL — but binding.** This is the canonical post-mortem of the
> two termination/critic-aliasing Elo collapses. Its central rule still governs
> the codebase: *whoever changes episode boundaries must let the critic learn the
> new return structure.* Cited from `src/ExampleMain.cpp` and `CLAUDE.md`.
> The steering mechanism itself is parked ([LADDER.md](LADDER.md) replaced it).
>
...
- `research/tools/derive_steering.py` — now an OFFLINE analysis tool only (the
