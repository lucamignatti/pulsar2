...
int main(int argc, char** argv) {
	...
	// GGL_MOE_TOPK must match training - it is a routing scalar, not a saved parameter.
	{
		...
		if (ReadMoEConfigFromModule(checkpoint + "/SHARED_HEAD.lt", sharedHeadConfig,
				(tk && *tk) ? std::atoi(tk) : 4)) {
			// Only the TRUNK's residual flag is an MoE consequence. The policy head keeps
			// the lineage default: addResiduals is load-invisible, so overriding it makes
			// a residual-head checkpoint play silently wrong (measured: 0-250 vs Nexto).
		}
	}
