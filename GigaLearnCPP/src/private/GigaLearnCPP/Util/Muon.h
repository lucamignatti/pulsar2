...
namespace GGL {
	...
	private:
		...
		static double AdjustedLR(double lr, const torch::Tensor& tensor) {
			auto rows = (double)tensor.size(0);
			auto cols = (double)(tensor.numel() / tensor.size(0));
		...
		static void StepAdamW(torch::Tensor& param, const torch::Tensor& grad, const MuonOptions& options, MuonParamState& state) {
			...
			auto biasCorrection1 = 1.0 - std::pow(beta1, state.step());
			auto biasCorrection2 = 1.0 - std::pow(beta2, state.step());
			auto stepSize = options.lr() / biasCorrection1;
